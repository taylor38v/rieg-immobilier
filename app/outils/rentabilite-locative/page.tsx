...
export default function Page() {
  ...
  const r = useMemo(() => {
    ...
    const evolution = Array.from({ length: 21 }, (_, y) => {
      ...
      const capitalRembourse = y === 0 ? 0 : Array.from({ length: y * 12 }, (_, m) => {
        const interet = (emprunt - 0) * r;
        return mensualite - interet;
      }).reduce((a, b) => a + b, 0) * 0.6;
      return { annee: y, valeur: valeurBien, cashflowCumul, capitalRembourse };
    });
  ...
  return (
    <div className="max-w-7xl mx-auto px-6 py-16 md:py-20">
      ...
      <div className="text-xs uppercase tracking-[0.3em] text-gold mt-8">Outil 07</div>
      <h1 className="font-serif text-5xl md:text-6xl mt-3">Rentabilité locative</h1>
      ...
      <div className="grid lg:grid-cols-[1fr_1.4fr] gap-12 mt-12">
        <div className="space-y-5">
          <Block title="Le bien">
            <Slider label="Prix d'achat" value={prix} onChange={setPrix} min={50000} max={1500000} step={5000} unit="€" />
            <Slider label="Loyer mensuel" value={loyer} onChange={setLoyer} min={300} max={5000} step={25} unit="€" />
          </Block>
          <Block title="Charges annuelles">
            <Slider label="Charges (copro, entretien)" value={chargesAn} onChange={setChargesAn} min={0} max={10000} step={50} unit="€" />
            <Slider label="Taxe foncière" value={tf} onChange={setTf} min={0} max={8000} step={50} unit="€" />
            <Slider label="Vacance locative" value={vacance} onChange={setVacance} min={0} max={20} step={1} unit="%" />
          </Block>
          <Block title="Financement">
            <Slider label="Apport" value={apport} onChange={setApport} min={0} max={Math.min(prix, 500000)} step={2000} unit="€" />
            <Slider label="Taux du prêt" value={taux} onChange={setTaux} min={1} max={6} step={0.05} unit="%" />
            <Slider label="Durée" value={duree} onChange={setDuree} min={10} max={25} step={1} unit="ans" />
            <Slider label="TMI" value={tmi} onChange={setTmi} min={0} max={45} step={11} unit="%" />
          </Block>
        </div>
        ...
        <div className="space-y-4">
          ...
          <div className="bg-navy text-ivory p-6">
            <div className="text-xs uppercase tracking-[0.3em] text-gold">Cash-flow mensuel après impôts</div>
          ...
          <div className="bg-white border border-ink/10 p-6">
            <div className="text-xs uppercase tracking-[0.3em] text-gold">Patrimoine constitué dans {duree} ans</div>
...
function Block({ title, children }: { title: string; children: React.ReactNode }) {
  return (
    <div className="p-5 bg-ivory-deep">
      <div className="text-xs uppercase tracking-[0.3em] text-gold mb-4">{title}</div>
